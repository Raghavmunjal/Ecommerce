...
import { listSubCategoryProducts } from "../Actions/productAction";
...
const ProductSubCategoryScreen = ({ match }) => {
  const productSubCategoryList = useSelector(
    (state) => state.productSubCategoryList
  );
  const { products, page, pages, loading } = productSubCategoryList;
  ...
  useEffect(() => {
    dispatch(listSubCategoryProducts(productSlug, pageNumber));
